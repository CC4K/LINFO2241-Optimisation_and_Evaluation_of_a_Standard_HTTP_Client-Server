...
PATTERNS_SIZE=64
RATE=-1
CFLAGS=( "-DSIMD128" "-DSIMD256" "-DSIMDBEST" ) # "-DSIMD512"

# Ensure the measurements directory exists
mkdir -p measurements

# No SIMD flag run
make -B run_release_simd NB_WORKER=1 &
for i in $(seq 1 $N); do
    env matsize=$MATSIZE patterns_size=$PATTERNS_SIZE nb_patterns=$NB_PATTERNS ./wrk2/wrk http://localhost:8888/ --duration 10s --rate $RATE \
    -s project/wrk_scripts/simple_scenario.lua > "output.csv"

    # Parse and append to main csv
    # TODO: python script for parsing Requests/sec
done

# SIMD128, SIMD256 (and SIMD512) run
for CFLAG in "${CFLAGS[@]}"; do
    make -B run_release_simd CFLAGS+="-$CFLAG" NB_WORKER=1 &
    for i in $(seq 1 $N); do
        env matsize=$MATSIZE patterns_size=$PATTERNS_SIZE nb_patterns=$NB_PATTERNS ./wrk2/wrk http://localhost:8888/ --duration 10s --rate $RATE \
        -s project/wrk_scripts/simple_scenario.lua > "output.csv"
...
#
#    for i in $(seq 1 $N); do
#        echo "Running iteration $i for CFLAG=$CFLAG"
#
